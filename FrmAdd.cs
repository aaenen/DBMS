...
namespace 教室信息管理系统
{
    public partial class FrmAdd : Form
    {
        ...
        {
            InitializeComponent();
            textBoxSerialNumber.Text = SearchSerialNumber();
        }
        ...
        private void buttonConfirm_Click(object sender, EventArgs e)
        {
            ...
            string strClassNumber = textBoxClassNumber.Text;
            int intClassNumber = int.Parse(textBoxClassNumber.Text);

            if (strClassroomID.Length == 0 || strCourseName.Length == 0 || strTeacherID.Length == 0 || strClassDate.Length == 0 || strClassNumber.Length == 0)
            {
            ...
            else
            {
                ...
                if (!blClassroomID)
                {
                    MessageBox.Show("该教室不存在，添加失败！", "信息提醒", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else if (!blClassDate)
                {
                    MessageBox.Show("该时间不合法，添加失败！", "信息提醒", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else if (!blCourseName)
                {
                    MessageBox.Show("该课程不存在，添加失败！", "信息提醒", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else if (!blTeacherID)
                {
                    MessageBox.Show("该教师不存在，添加失败！", "信息提醒", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else if(!blClassNumber)
                {
                    MessageBox.Show("该上课人数大于该课室可容纳人数，添加失败！", "信息提醒", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }    
                else
                {
                    string message = InsertData(strClassroomID, strCourseName, strTeacherID, strClassDate, intClassNumber);
        ...
        ///
        private string InsertData(string strClassroomID, string strCourseName, string TeacherID, string ClassDate,int ClassNumber)
        {
            int serialnumber = int.Parse(textBoxSerialNumber.Text);
            string strClassweek = BuildWeekday(ClassDate);
            string strSQL = @"INSERT INTO [dbo].[classroommanage]
                                           ([serialnumber]
                                           ,[classroomid]
                                           ,[coursename]
                                           ,[teacherid]
        ...
        private string SearchSerialNumber()
        {
            ...
            string str = dataTable.Rows[dataTable.Rows.Count - 1][0].ToString();
            string i = (int.Parse(str) + 1).ToString();
            return i;
        }
    }
}
